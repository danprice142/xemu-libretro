...
/* Expose bound_controllers and bound_drivers required by xemu-input.h */
...
/* Keyboard scancode map — required by xemu-input.h but unused in libretro */
...
void xemu_input_update_controller(ControllerState *state)
{
    ...
    /* Digital buttons: RetroPad → Xbox controller mapping
     * Xbox A = RetroPad B (south face button)
     * Xbox B = RetroPad A (east face button)
     * Xbox X = RetroPad Y (west face button)
     * Xbox Y = RetroPad X (north face button)
     * Xbox White = RetroPad L
     * Xbox Black = RetroPad R
     */
    ...
    /* Analog triggers: L2/R2 → Xbox left/right trigger
     * Xbox triggers are 0-32767 range via axis[] */
    ...
    /* Analog sticks - invert Y axes to match Xbox convention (positive=up, negative=down) */
...
/* Create XID USB gamepad devices for all 4 ports.
 * Must be called after qemu_init() with BQL held. */
void libretro_input_create_xid_devices(void)
{
    ...
    for (int i = 0; i < 4; i++) {
        ...
        /* Create XID controller on port 1 of the hub */
...
/* Stubs for functions not needed in libretro mode */
...
{
    (void)f;
    return true;  /* No extra data to skip in libretro build */
}

void xemu_snapshots_save_extra_data(QEMUFile *f)
{
    (void)f;
    /* No extra data to write in libretro build */
}
